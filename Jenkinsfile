    ...
    stages {
        ...
        stage('docker run mirror 1') {
            steps {
                ...
                sh "docker run -d --name usuario-service-1 -it --cpu-shares='256' --memory='512m' --network host guillegregoret/usuarios"
            }
        }
        stage('docker run mirror 2') {
            steps {
                ...
                sh "docker run -d --name usuario-service-2 -it --cpu-shares='256' --memory='512m' --network host guillegregoret/usuarios"
            }
        }
        stage('docker run mirror 3') {
            steps {
                ...
                sh "docker run -d --name usuario-service-3 -it --cpu-shares='256' --memory='512m' --network host guillegregoret/usuarios"
            }
        }
    }
}
